...
export const Cards = ({animate=true}) => {
    return <div className="my-container relative lg:min-h-32  z-30 -mt-15 lg:-mt-24 flex flex-col lg:flex-row gap-8 justify-between ">
            {cardinfo.map((item,key)=>{
                ...
                " bg-white/60 group-hover:bg-amber-100/60    backdrop-blur-lg lg:w-48 rounded-4xl shadow-center flex flex-col items-center text-center hover:inner-shadow-center transition-colors duration-300")}>
                        ...
                        <img src={item.img} className='absolute top-1 -left-1 inset-0 object-cover h-full xs blur-[2px] -z-10  rounded-4xl group-hover:opacity-20 opacity-0 transition-all duration-300 '></img>
                    </ScrollLink>
