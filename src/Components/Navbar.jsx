    ...
    >
        <div className = "my-container mx-auto px-4 flex  items-center justify-between" >
                ...
                {navItems.map((item,dkey)=>(
                    <div key={dkey} className="relative  group ">
                        <ScrollLink to={item.href} className={
                            cn("text-white hover:text-secondary font-medium transition-colors  duration-250 border-b-2 border-transparent  flex items-center",
                            ...
                            <div className=" -left-5 group-hover:opacity-100 group-hover:pointer-events-auto rounded-xl bg-primary shadow-2xl 
                                    ...
                                    {item.subMenu.map((sub, subKey) => (
                                        <div key={subKey} className="relative group/sub">
                                            {sub.subMenu ? (
                                                <>
                                                    <div className="px-5 py-4 text-white rounded-xl hover:text-secondary  flex items-center justify-between w-60">
                                                        {sub.name}
                                                        <ChevronDown size={16} />
                                                    </div>
                                                    <div className="absolute left-full top-0 mt-0  w-60 bg-primary rounded-xl shadow-lg opacity-0 group-hover/sub:opacity-100 pointer-events-none group-hover/sub:pointer-events-auto transition duration-300 z-30">
                                                        {sub.subMenu.map((deep, idx) => (
                                                            <ScrollLink
                                                                key={idx}
                                                                to={deep.href}
                                                                className="block px-5 py-4 rounded-xl text-sm text-white hover:text-secondary"
                                                            >
                                                                {deep.name}
                                                            </ScrollLink>
                                                        ))}
                                                ...
                                                <ScrollLink
                                                    to={sub.href}
                                                    className="block px-5 py-4 w-60 text-white rounded-xl hover:text-secondary"
                                                >
                                                    {sub.name}
                                                </ScrollLink>
                                            )}
            ...
            <button onClick={()=>setisMenuOpen((prev) => !prev)} 
                className={cn("xl:hidden p-2 z-50  text-white")}> 
                {isMenuOpen
                        ...
                        {navItems.map((item, mkey) => (
                            ...
                            >
                                <ScrollLink to={item.href} className="flex items-center text-left justify-center   gap-4 ">
                                    <h1 className="text-left hover:text-secondary ">
                                        {item.name}
                                    </h1>
