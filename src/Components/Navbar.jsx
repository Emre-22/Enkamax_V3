    ...
    >
        <div className = "container mx-auto px-4 flex items-center justify-between" >
            ...
            <div className="hidden md:flex space-x-8">
                {navItems.map((item,dkey)=>(
                    <div key={dkey} className="relative group">
                        <Link to={item.href} className="text-gray-600 hover:text-teal-600 font-medium transition-colors duration-300 border-b-2 border-transparent hover:border-amber-200">{item.name}</Link>

                        {/* Submenu */}
                        {item.subMenu && (
                        <div className="absolute left-1/2 -translate-x-1/2 mt-0 w-30 text-center bg-white shadow-lg pointer-events-none  rounded-md opacity-0 group-hover:opacity-100 group-hover:pointer-events-auto transition-all duration-300">
            ...
            <button onClick={()=>setisMenuOpen((prev) => !prev)} 
                ...
                <div className={cn("fixed inset-0 bg-teal-900/95 backdrop-blur-md z-40 flex flex-col items-center text-center  justify-center transition-none  md:hidden",
                    ...
                    <div className="flex flex-col w-full text-center items-center  justify-center space-y-8 text-xl">
                        ...
                        <div key={mkey} className="w-full">
                            ...
                            >
                                <Link to={item.href} className="flex items-center justify-center  w-[200px] gap-4 ">
                                    {item.name}
